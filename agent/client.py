...
class TorqueTuneAgent:
    ...
    @property
    def short_term(self):  return self.memory.short_term
    @property
    def scratchpad(self):  return self.memory.scratchpad
    @property
    def router(self):      return self.memory.router
    @property
    def episodic(self):    return self.memory.episodic
    @property
    def semantic(self):    return self.memory.semantic
    @property
    def consolidator(self): return self.memory.consolidator
    @property
    def rag(self):         return self.memory.hybrid_rag
    @property
    def agentic_rag(self): return self.memory.agentic_rag
    ...
    async def __aenter__(self) -> TorqueTuneAgent:
        ...
        self.session = await self._exit_stack.enter_async_context(
            ClientSession(
                read,
                write,
                sampling_callback=lambda ctx, params: agent.sampling.handle_sampling(self, params),
                elicitation_callback=lambda ctx, params: agent.elicitation.handle_elicitation(self, params),
...
"""
        ...
        kb = (await self.memory.retrieve(user_message)
              if self.memory.needs_knowledge(user_message) else None)
        ...
        while True:
            ...
            for call in response.function_calls:
                ...
                args = dict(call.args) if call.args else {}
                result = await call_tool_with_progress(self, call.name, args)
                function_responses.append(gtypes.Part.from_function_response(
                    name=call.name,
                    response={"content": tool_result_to_text(result),
                              "is_error": result.isError or False},
                ))
                # HOOK 6 — tool result enters buffer; overflow fires promote-or-drop
                self.memory.ingest("tool", tool_result_to_text(result),
