...
class Graph1:
    ...
    def step(self, state: RunState) -> RunState:
        ...
        # Set the correct status immediately when entering a waiting state.
        if state.current_node == self.WAITING_INSPECTION:
            state.status = RunStatus.WAITING_EXTERNAL

        elif state.current_node == self.WAITING_HITL:
            state.status = RunStatus.WAITING_HITL

        elif state.current_node == self.TICKET_OPEN:
            state.status = RunStatus.TICKET_OPEN

        else:
            state.status = RunStatus.RUNNING

        # Persist checkpoint after the transition.
        self.checkpoints.save(state)
    ...
    def submit_inspection_result(
        ...
        An inconclusive result opens a ticket because the
        investigation cannot be resolved through a normal retry.
        """
    ...
    def resume(self, run_id: str) -> RunState:
        ...
        Waiting states are returned without automatically
        advancing because they require an external event,
        HITL decision, or ticket resolution.
        """

        state = self.checkpoints.load(run_id)

        # Do not automatically pass states that require
    ...
    def get_state(self, run_id: str) -> RunState:
        """Return the latest saved checkpoint."""
